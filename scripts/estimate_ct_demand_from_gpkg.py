...
def estimate_ct_demand_from_gpkg():
    """
    Compute total estimated CT demand per LSOA using age-segmented population
    and national CT demand rates by age band.

    Returns:
    - DataFrame with LSOA code and estimated CT total
    """
    ...
    # --- Step 1: Load age-based CT counts and population ---
    ...
    df = modality_df[["age_band", "CT_Total"]].merge(pop_df, on="age_band")
    ...
    # Create dict: LSOA column → per-1k rate
    ct_rate_dict = {
        ageband_to_column[ab]: rate
        for ab, rate in zip(df["age_band"], df["ct_demand_per_1k"])
    }

    # --- Step 3: Load GPKG and compute CT demand per LSOA ---
    gdf = gpd.read_file(gpkg_path)

    for col, rate in ct_rate_dict.items():
        gdf[f"ct_{col}"] = (gdf[col] * rate) / 1000

    ct_cols = [f"ct_{col}" for col in ct_rate_dict.keys()]
    gdf["ct_total_demand"] = gdf[ct_cols].sum(axis=1)

    return gdf[["lsoa21cd", "ct_total_demand"] + ct_cols]
